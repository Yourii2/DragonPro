...
const PrintWaybill: React.FC = () => {
  ...
  useEffect(() => {
    ...
    let companyLogo = q.get('companyLogo') || '';
    let companyAddress = q.get('companyAddress') || '';
      ...
      if (!companyName && !companyPhone && !companyTerms) {
        try {
          ...
          if (svj && svj.status === 'ok' && svj.settings) {
            ...
            companyAddress = svj.settings.company_address || '';
            if (svj.settings.company_logo_url) companyLogo = svj.settings.company_logo_url;
            else if (svj.settings.company_logo) companyLogo = svj.settings.company_logo;
          }
        } catch (e) {
        ...
        for (const id of ids) {
          try {
            ...
            const j = await r.json();
              ...
              o.companyLogo = companyLogo || o.companyLogo || localStorage.getItem('Dragon_company_logo') || '';
              o.companyAddress = companyAddress || o.companyAddress || localStorage.getItem('Dragon_company_address') || '';
              fetched.push(o);
            } else {
              ...
              <PageStyle key={idx}>
                <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 8 }}>
                  <div className="company" style={{ flex: 1 }}>
                      {companyLogoSrc ? <img src={companyLogoSrc} alt="logo" style={{ maxWidth: 160, height: 'auto', marginBottom: 6 }} /> : null}
                      <div style={{ fontWeight: 800, fontSize: 18 }}>{companyName}</div>
                      <div style={{ fontWeight: 700 }}>{companyPhone}</div>
                      {companyAddressStr ? <div style={{ fontSize: 12, color: '#444' }}>{companyAddressStr}</div> : null}
