...
if ($action === 'check') {
    ...
    try {
        ...
        $selectable = array_values(array_filter($ctx['releases'], fn($r) => !empty($r['asset_url'])));
        ...
        echo json_encode([
            ...
            'pending' => $newWithZip,
            'selectable_count' => count($selectable),
...
if ($action === 'run') {
    ...
    try {
        ...
        $ctx = get_update_context($root);
        $queue = array_values(array_filter($ctx['releases'], fn($r) => !empty($r['asset_url'])));
        ...
        if (count($queue) === 0) {
            echo json_encode(['success' => true, 'message' => 'لا توجد إصدارات محددة قابلة للتثبيت.', 'installed' => []]);
            @unlink($lockPath);
            exit;
        }
  ...
  <script>
    ...
    const btnRun = document.getElementById('btnRun');
        ...
        function syncButtons(){
            ...
            btnSelectAll.disabled = !hasSelectable;
            btnClearAll.disabled = !hasSelectable;
      ...
      try{
        ...
        if(!j.success) throw new Error(j.message || 'فشل فحص التحديثات');
                            ...
                            pendingRows = (j.pending || []);
                                        selectableRows = (j.selectable || []);
                                        // Pre-select pending releases by default so the installer will run them all at once
                                        selectedTags = new Set((j.pending || []).map(r => String(r.tag || '')));
                            statusEl.innerHTML = `الإصدار الحالي: <b>${j.current_version}</b> — تحديثات متاحة: <b>${j.pending_count}</b> — إصدارات قابلة للتثبيت: <b>${j.selectable_count || 0}</b> — إجمالي الإصدارات: <b>${(j.all || []).length}</b>`;
          ...
          div.className = 'row';
                    ...
                    div.innerHTML = `
                        <div style="display:flex;align-items:center;gap:8px;">
                            <input type="checkbox" data-tag="${tag}" ${selectedTags.has(tag) ? 'checked' : ''} />
          ...
          listEl.appendChild(div);
            ...
            allHead.innerHTML = '<div><b>كل الإصدارات (اختر أي إصدار قديم أو حديث)</b></div>';
                ...
                (j.all || []).forEach(r => {
                    ...
                    if (hasZip) {
                        div.innerHTML = `
                            ...
                            <div><span class="${statusClass}">${statusLabel}</span> | <span class="${zipClass}">${zipLabel}</span></div>
                        `;
                        const checkbox = div.querySelector('input[type="checkbox"]');
                        if (checkbox) {
                            checkbox.addEventListener('change', (e) => {
    ...
    async function run(){
      ...
      try{
        ...
        btnSelectAll.addEventListener('click', () => {
            selectedTags = new Set(selectableRows.map(r => String(r.tag || '')));
