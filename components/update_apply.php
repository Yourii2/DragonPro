...
try {
    ...
    // Accept either a single asset_url or an array of asset_urls for batch installs
    $rawUrls = $input['asset_urls'] ?? ($input['asset_url'] ?? null);
    $assetUrls = [];
    if (is_array($rawUrls)) {
        ...
        $assetUrls[] = trim($rawUrls);
    }

    if (count($assetUrls) === 0) {
        http_response_code(400);
        echo json_encode(['success' => false, 'message' => 'asset_url or asset_urls is required']);
        exit;
    }
    ...
    foreach ($assetUrls as $assetUrl) {
        if (!preg_match('#^https://#i', $assetUrl)) {
            http_response_code(400);
            echo json_encode(['success' => false, 'message' => 'Only https:// URLs are allowed: ' . $assetUrl]);
            exit;
        }
        if (!host_allowed($assetUrl, $allowedHosts)) {
            http_response_code(400);
            echo json_encode(['success' => false, 'message' => 'Download host is not allowed: ' . $assetUrl]);
            exit;
        }
    }
    ...
    foreach ($assetUrls as $assetUrl) {
        ...
        $backupZips[] = basename($zipPath);
        // slight delay to avoid timestamp collisions if loop is very fast
        usleep(200000);
    }
