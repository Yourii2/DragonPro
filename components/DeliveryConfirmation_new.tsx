// كارت عرض إذن تسليم بسيط
const DeliveryNoteCard = ({ note, onSelect }: { note: any, onSelect?: (note: any) => void }) => (
  <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-200 mb-3 flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div className="flex-1">
      <div className="flex items-center gap-3 mb-2">
        <span className="bg-indigo-100 text-indigo-700 px-3 py-1 rounded-lg font-bold text-sm">
          #{note.code || note.note_code || note.id}
        </span>
      ...
      <div className="flex flex-col gap-1 text-slate-700">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold">{note.rep_name || note.to_warehouse_name || 'مندوب غير محدد'}</span>
        </div>
        <div className="flex items-center gap-2">
          <Warehouse className="w-4 h-4 text-emerald-600" />
          <span className="font-bold">{note.warehouse_name || note.from_warehouse_name || 'مخزن غير محدد'}</span>
        </div>
        <div className="flex items-center gap-2">
          <Package className="w-4 h-4 text-blue-600" />
          <span className="font-bold">{note.total_items || note.items?.length || 0} قطعة</span>
        </div>
      </div>
    </div>
    {onSelect && (
...
const DeliveryConfirmation: React.FC = () => {
  ...
  const loadNote = async (id?: string) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE_PATH}/api.php?module=dispatch&action=getDeliveryNote&id=${encodeURIComponent(useId)}`);
  ...
  const handleManualScan = (e: React.FormEvent) => {
    ...
    const foundIdx = items.findIndex(it => String(it.product_barcode) === scanValue.trim());

    if (foundIdx !== -1) {
      if (items[foundIdx].scanned_qty < items[foundIdx].qty_sent) {
  ...
  const saveScans = async () => {
    setLoading(true);
    try {
      // تحويل البيانات للتنسيق المطلوب من confirmReceiptV2
      const itemsPayload = note.items.map((item: any) => ({
        item_id: item.id,
        factory_product_id: item.factory_product_id,
        qty_received: item.scanned_qty
      }));

      const res = await fetch(`${API_BASE_PATH}/api.php?module=dispatch&action=confirmReceiptV2`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ order_id: note.id, items: itemsPayload })
      });
      const data = await res.json();
      if (data.success) {
  ...
  const totalRequired = note?.items.reduce((s: number, i: any) => s + Number(i.qty_sent), 0) || 0;
            ...
            <div className="lg:col-span-2 bg-white rounded-3xl p-6 shadow-sm border border-slate-100 flex flex-col md:flex-row gap-8 justify-around relative overflow-hidden">
               ...
               <div className="relative flex items-center gap-4">
                 ...
                 <div>
                   ...
                   <span className="text-lg font-black text-slate-700">{note.to_warehouse_name || 'غير محدد'}</span>
                 </div>
               </div>
               ...
               <div className="relative flex items-center gap-4">
                 ...
                 <div>
                   ...
                   <span className="text-lg font-black text-slate-700">{note.from_warehouse_name || 'الرئيسي'}</span>
                 </div>
               </div>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-slate-50">
                {note.items.map((it: any) => {
                  const isDone = it.scanned_qty === it.qty_sent;
                    ...
                    >
                      ...
                      <td className="p-5 text-center">
                        <div className="flex gap-2 justify-center">
                          ...
                          <span className="bg-indigo-50 px-3 py-1 rounded-lg text-xs font-bold text-indigo-600">{it.size_name || it.size || '—'}</span>
                        </div>
                      </td>
                      <td className="p-5 text-center font-mono text-sm text-slate-400">{it.product_barcode}</td>
                      <td className="p-5 text-center font-black text-slate-500">{it.qty_sent}</td>
                      <td className="p-5 text-center">
