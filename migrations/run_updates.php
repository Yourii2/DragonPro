...
// Utility: truncate all base tables and reset AUTO_INCREMENT to 1
function truncateAllTablesAndReset(PDO $pdo) {
    echo "\n*** Truncating all tables and resetting AUTO_INCREMENT to 1 (FORCE_ZERO_TABLES) ***\n";
    try {
        $tables = $pdo->query("SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_TYPE = 'BASE TABLE'")->fetchAll(PDO::FETCH_COLUMN);
        if (!$tables) {
        ...
        foreach ($tables as $t) {
            try {
                ...
                echo "WARN: failed to truncate $t: " . $e->getMessage() . "\n";
            }
        }
        // Re-enable foreign key checks
        $pdo->exec('SET FOREIGN_KEY_CHECKS = 1');
        echo "  All tables truncated. AUTO_INCREMENT values reset by TRUNCATE.\n";
    } catch (Exception $e) {
...
// IMPORTANT: Do NOT truncate tables during routine updates — truncation is destructive and should only run
// when a fresh database is being installed. To avoid accidental data loss, we do NOT auto-run truncation
// here even if an environment flag exists. If you need to zero tables during a fresh install, invoke the
// truncation explicitly from the installer (components/setup.php) or run the script manually with FORCE_ZERO_TABLES.

// Apply performance indexes to optimize high-traffic tables
...
foreach ($perfIndexes as $pIdx) {
    ...
    $cols = $pIdx[2];
    try {
        $checkTbl = $pdo->query("SHOW TABLES LIKE '$tbl'")->fetch();
        if (!$checkTbl) continue;
        if (!indexExists($pdo, $tbl, $idxName)) {
            $pdo->exec("ALTER TABLE `$tbl` ADD INDEX `$idxName` ($cols)");
            echo "  ✅ Added index $idxName on $tbl ($cols)\n";
        } else {
            echo "  ℹ️ Index $idxName on $tbl already exists\n";
        }
    } catch (Exception $e) {
...
// For update runs we only reset AUTO_INCREMENT for empty tables so fresh installs still get IDs starting at 1
resetAutoIncrementForEmptyTables($pdo);
