  ...
  for (let line of lines) {
    ...
    if (line.startsWith('`')) {
      ...
      if (m) {
        const col = m[1];
        const rest = m[2];
        // simple parse: type, nullability, default
...
function applyAlterStatements(schema, sql) {
  // process simple ALTER TABLE ADD COLUMN / ADD KEY / ADD CONSTRAINT
  ...
  while ((m = alterRegex.exec(sql))) {
    ...
    // split multiple ALTER actions by ","
...
function compareSchemas(base, current) {
  ...
  for (const t of common) {
    ...
    // indexes: naive compare by names
    ...
    if (cd.missing_columns.length || cd.extra_columns.length || cd.changed_columns.length || cd.missing_indexes.length || cd.extra_indexes.length || cd.pk_diff || cd.fk_diff.length) {
      report.diffs[t] = cd;
    }
  }
  return report;
