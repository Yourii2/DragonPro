    ...
    case 'sales':
        ...
        if ($action === 'getConfirmationAssignments') {
            try {
                ...
                // Auto-clear assignments from previous days (daily reset)
                execute_query(
                    $pdo,
                ...
                $repId = intval($_GET['rep_id'] ?? 0);
                $where = ["oca.status = 'assigned'", 'o.id IS NOT NULL', 'DATE(oca.assigned_at) = CURDATE()'];
                $params = [];
                if ($repId > 0) {
                    $where[] = 'oca.rep_id = ?';
                    $params[] = $repId;
                ...
                if (column_exists($pdo, 'orders', 'status')) {
                    $where[] = "o.status IN ('pending','returned','postponed')";
                }
        ...
        if ($action === 'updateOrderConfirmationDecision') {
            try {
                ...
                if ($decision === 'cancel') {
                    ...
                    $statusEnums = get_enum_values($pdo, 'orders', 'status');
                    if (!in_array('cancelled', $statusEnums, true)) {
                        throw new Exception('لا يمكن إلغاء الأوردر لأن حالة cancelled غير معرفة في عمود status بجدول orders.');
                    }
