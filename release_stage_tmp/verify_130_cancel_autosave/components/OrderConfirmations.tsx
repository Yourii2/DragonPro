...
import { CheckSquare, MapPin, Package2, Phone, PhoneCall, Printer, RefreshCw, Save, ScanLine, ShieldCheck, Trash2, UserCheck, Users2, WalletCards, XCircle } from 'lucide-react';
...
const OrderConfirmations: React.FC = () => {
  ...
  const [recentCancelled, setRecentCancelled] = useState<AssignmentRow[]>([]);
  ...
  const activeSelectedRepOrders = useMemo(() => selectedRepOrders, [selectedRepOrders]);

  const cancelledOrders = useMemo(
    () => recentCancelled.map((assignment) => ({ assignment, decision: 'cancel' as const })),
    [recentCancelled]
  );
  ...
  const handleDecisionBarcode = async (decision: 'cancel') => {
    ...
    try {
      ...
      setRecentCancelled((prev) => [assignment, ...prev.filter((x) => Number(x.order_id) !== Number(assignment.order_id))].slice(0, 50));
          ...
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                className="inline-flex items-center gap-1 rounded-xl bg-blue-600 px-3 py-2 text-xs font-black text-white transition-colors hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60"
              >
              ...
              <button
                ...
                className="inline-flex items-center gap-1 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-black text-rose-700 transition-colors hover:bg-rose-100 disabled:cursor-not-allowed disabled:opacity-60 dark:border-rose-900/40 dark:bg-rose-950/30 dark:text-rose-200 dark:hover:bg-rose-950/50"
              >
