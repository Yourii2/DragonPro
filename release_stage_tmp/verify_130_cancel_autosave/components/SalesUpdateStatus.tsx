...
const SalesUpdateStatus: React.FC = () => {
  ...
  const [returnItems, setReturnItems] = useState<Array<{ productId:number; name:string; quantity:number }>>([]);
    ...
    try {
      ...
      setOpenRepOrders((prev:any) => {
        ...
        const newOrders = (prev.orders||[]).map((o:any) => {
          ...
          // if order wasn't returned partially (maybe fully returned and server didn't return order), remove it later
          return o;
        }).filter((o:any)=> {
          const u = updatedOrdersMap[o.id];
          if (u) return true;
          // if this id was in selected ids but not in updatedOrdersMap, assume fully returned => remove
