...
if ($action === 'run') {
    ...
    try {
        ...
        if (count($queue) === 0) {
            echo json_encode(['success' => true, 'message' => 'لا توجد تحديثات جديدة قابلة للتثبيت.', 'installed' => []]);
            @unlink($lockPath);
            exit;
        }
        ...
        @unlink($lockPath);
        echo json_encode(['success' => true, 'message' => 'تم تثبيت التحديثات تلقائياً بنجاح.', 'installed' => $installed]);
    } catch (Exception $e) {
        @unlink($lockPath);
        http_response_code(500);
        echo json_encode(['success' => false, 'message' => $e->getMessage()]);
...
<body>
  <div class="card">
    ...
    <div style="margin:14px 0;display:flex;gap:8px;flex-wrap:wrap;">
      <button id="btnRun" disabled>تثبيت كل التحديثات تلقائياً</button>
  ...
  <script>
      ...
      try{
        ...
        if(!j.success) throw new Error(j.message || 'فشل فحص التحديثات');
                ...
                if ((j.pending || []).length > 0) {
                    ...
                    head.className = 'row';
                    head.innerHTML = '<div class="warn"><b>الإصدارات الجديدة القابلة للتثبيت الآن</b></div>';
          ...
          div.className = 'row';
          div.innerHTML = `<div><b>${r.version}</b> - ${r.name}</div><div class="warn">${r.asset_name || 'ZIP'}</div>`;
          listEl.appendChild(div);
                ...
                btnRun.disabled = (j.pending_count || 0) === 0;
      }catch(e){
    ...
    async function run(){
      ...
      addLog('بدء التثبيت التلقائي...');
      try{
        const res = await fetch('?action=run', { method:'POST', credentials:'include' });
        const j = await res.json();
