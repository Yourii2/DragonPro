...
// Ensure table exists
$pdo->exec("CREATE TABLE IF NOT EXISTS backup_email_otps (
...
$stmt = $pdo->prepare("SELECT id FROM backup_email_otps WHERE user_id = ? AND email = ? AND verified_at IS NULL LIMIT 1");
...
$ins = $pdo->prepare("INSERT INTO backup_email_otps (user_id, email, code, expires_at) VALUES (?, ?, ?, ?)");
$ins->execute([$uid, $email, $code, $expires]);

$subject = 'كود التحقق - النسخ الاحتياطي';
