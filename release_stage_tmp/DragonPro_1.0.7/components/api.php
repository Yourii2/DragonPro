    ...
    case 'permissions':
        ...
        if (!can_manage_permissions($pdo)) {
            ...
            if (in_array($action, $alwaysReadableActions, true)) {
                ...
                // If user_id not provided (or is 0), assume current user.
                ...
                if ($status !== null) {
                    try {
                        ...
                        if ($repForTx) {
                            ...
                            if ($status === 'delivered') {
                                // Consignment model: delivered does NOT create any financial entries for the Rep.
                            }
                            ...
                            if ($status === 'in_delivery' && isset($input['deliveredAmount'])) {
                                ...
                                // Delivered portion: no financial entry in consignment model.
                                if ($returnedAmt > 0) {
                                    $txType = pick_allowed_enum($pdo, 'transactions', 'type', 'rep_return_credit', ['rep_return_credit','rep_settlement','payment_in','sale']);
                                    execute_query($pdo, "INSERT INTO transactions (type, warehouse_id, treasury_id, related_to_type, related_to_id, amount, transaction_date, details) VALUES (?, ?, ?, ?, ?, ?, NOW(), ?)", [$txType, null, null, $relType, $repForTx, $returnedAmt, json_encode(['order_id'=>$id,'action'=>'partial_returned','returned'=>$returnedAmt,'delivered'=>$deliveredAmt,'model'=>'consignment'])]);
    ...
    case 'sales':
        ...
        if ($action === 'completeDaily') {
            ...
            try {
                ...
                // Rep balance is computed as SUM(transactions.amount) for related_to_type='rep'. Negative means debt.
